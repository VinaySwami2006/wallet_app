...
__export(src_exports, {
  ...
  useRestoreAccessibilityFocus: () => useRestoreAccessibilityFocus
});
...
function useAccessibilityFocus(enabled = true) {
  ...
  React.useEffect(() => {
    ...
    const timeout = setTimeout(() => {
      const node = (0, import_react_native.findNodeHandle)(ref.current);
      if (node != null) {
        import_react_native.AccessibilityInfo.setAccessibilityFocus(node);
      }
    }, 50);
    return () => {
...
// src/useRestoreAccessibilityFocus.tsx
var React2 = __toESM(require("react"));
var import_react_native2 = require("react-native");
function useRestoreAccessibilityFocus(open, ref) {
  const wasOpenRef = React2.useRef(open);
  React2.useEffect(() => {
    const wasOpen = wasOpenRef.current;
    wasOpenRef.current = open;
    if (import_react_native2.Platform.OS === "web" || !wasOpen || open) {
      return;
    }
    const node = ref.current ? (0, import_react_native2.findNodeHandle)(ref.current) : null;
    if (node == null) {
      return;
    }
    const timeout = setTimeout(() => import_react_native2.AccessibilityInfo.setAccessibilityFocus(node), 50);
...
function useAugmentedRef({ ref, methods, deps = [] }) {
  const augmentedRef = React3.useRef(null);
...
function useRelativePosition({
  ...
  const dimensions = import_react_native3.Dimensions.get("screen");
...
function useIsomorphicLayoutEffect(effect, dependencies) {
  if (import_react_native4.Platform.OS === "web" && typeof window === "undefined") {
    React6.useEffect(effect, dependencies);
  } else {
    React6.useLayoutEffect(effect, dependencies);
  }
}

// src/use-compose-refs.ts
